...
final class IOService {
	...
	/* package */ static WordList loadWordList(String id) throws IOException {
		final int MAGIC = 0x776c6462;
		final String filename = "dict/" + id + ".bin";

		InputStream is = Gdx.files.internal(filename).read();
		if (read_int32(is) != MAGIC)
			throw new IOException("Language file is corrupted :(");

		int size_words = read_int32(is);
		int count_charset = read_int32(is);
		int count_words = read_int32(is);
		...
		System.out.println("Loading " + name + ", " + count_words + " words in " + count_charset + " letters and "
				+ size_words + " bytes");

		byte[] words = new byte[size_words];
		is.read(words);

		Map<Character, Character> map2utf8 = new TreeMap();
		Map<Character, Character> map2ascii = new TreeMap();
		for (int i = 0; i < count_charset; i++) {
			char code = (char) is.read();
			int flags = is.read();
			int count = read_int32(is);
			char unicode = (char) read_int32(is);
			map2utf8.put(code, unicode);
			map2ascii.put(unicode, code);
			System.out.printf("%3d: %c -> %c\n", i, code, unicode);
		}

		WordList wl = new WordList(id, name, words, map2ascii, map2utf8);
		System.out.println("Loading done");
		return wl;
		// return new WordList(name, words, map2ascii, map2utf8);
	}
	...
	/* package */ static Level[] loadLevels(WordList wl) throws IOException {
		final String filename = "levels/level." + wl.getId() + ".txt";
	...
	private static String getSaveFile(WordList wl, Level l) {
		return BASE + "/levelprog_" + wl.getId() + "_" + l.board;
	}

	/* package */ static void loadLevelProgress(WordList wl, Level l) throws IOException {
	...
	/* package */ static void saveLevelProgress(WordList wl, Level l) throws IOException {
		final String filename = getSaveFile(wl, l);
		FileHandle file = Gdx.files.local(filename);
		if (file == null) {
	...
	/* package */ static void deleteProgress(WordList wl, Level l) throws IOException {
		final String filename = getSaveFile(wl, l);
		FileHandle file = Gdx.files.local(filename);
		if (file != null)
			file.delete();
	...
	public static void loadSettings() {
		try {
			...
			for (;;) {
				...
				if (line == null)
					break;
				if ("sound_on".equals(line))
					Settings.sound_on = true;
				else if ("sound_off".equals(line))
	...
	private static final int read_int32(InputStream is) throws IOException {
		int ret = 0;
		for (int i = 0; i < 4; i++) {
			int c = is.read() & 0xFF;
			ret = (ret << 8) | c;
